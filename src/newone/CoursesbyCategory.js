...
import { FaRegClock, FaCode, FaChevronDown, FaChevronRight } from 'react-icons/fa';
...
const CoursesByCategory = () => {
    ...
    const renderPagination = (categoryCourses) => {
        const totalPages = Math.ceil(categoryCourses.length / coursesPerPage);
                ...
                <ul className="pagination">
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                        <li key={page} className={`page-item ${currentPage === page ? "active" : ""}`}>
                            <button
                                className={`page-link ${currentPage === page ? "bg-meroon" : "textcolor border-danger"}`}
                ...
                <div className="row">
                    <div className="col-12">
                        {categories.map((category) => {
                            ...
                            const currentCategoryCourses = categoryCourses.slice(startIdx, startIdx + coursesPerPage);
                                            ...
                                            {categoryCourses.length === 0 ? (
                                                ...
                                                <>
                                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                                        {currentCategoryCourses.map(({ _id, name, image, category, duration, logoImage, mode, description }) => (
                                                            <div
                                                                key={_id}
                                                    ...
                                                    {renderPagination(categoryCourses)}
                                                </>
                                            )}
                                        </div>
                                    )}
