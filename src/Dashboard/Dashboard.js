...
const Dashboard = () => {
  ...
  const fetchRecommendedCourses = async () => {
    try {
      const response = await axios.get(`https://hicap-backend-4rat.onrender.com/api/recommend-courses/${student.id}`);
  ...
  const fetchTodaysClasses = async () => {
    try {
      const response = await axios.get(`https://hicap-backend-4rat.onrender.com/api/live-classes/user/${student.id}`);
